...
const updateProductInCart = async (user, productId, quantity) => {
  ...
  if(!getCart)
    ...
    //Check if the product is present in our collection of MongoDB database.
  const products = await productService.getProductById(productId);
  if (!products) {
    throw new ApiError(httpStatus.BAD_REQUEST, "Product doesn't exist in database");
  }
  
  //Check if the product already present in the cart to update
  const cartData = await Cart.findOne({email: user.email});
  let check = false //Boolean variable to track the product found
  if(cartData){
    const cartItemsObject = cartData.cartItems;
    cartItemsObject.forEach((item) => {
      if(item.product.name == products.name.toLowerCase())
        check = true;   
  })
  if (!check)
  ...
  try{
    let cartData = await Cart.findOne({email: user.email});
    let itemIndex = cartData.cartItems.findIndex(p => p.product.name == products.name.toLowerCase());
    if (itemIndex > -1) {
      //product exists in the cart, update the quantity
      let productItem = cartData.cartItems[itemIndex];
      productItem.quantity = quantity;
      cartData.cartItems[itemIndex] = productItem;
    }
    await cartData.save();
    return cartData
  }catch(error){
