...
const updateProductInCart = catchAsync(async (req, res) => {
  ...
  if(quantities > 0){
    const updatedCart = await cartService.updateProductInCart(user, productId, quantities);
    res.status(httpStatus.OK).send(updatedCart);
  }else{
    const updatedCart = await cartService.deleteProductFromCart(user, productId);
    res.status(httpStatus.NO_CONTENT).send(updatedCart);
  }
});
